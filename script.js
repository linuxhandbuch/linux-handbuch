...
function renderCommandList() {
    ...
    if (searchQuery.trim() !== '') {
        // Suchmodus: über alle Kategorien hinweg filtern
        const results = [];
        categories.forEach((cat, catIndex) => {
            cat.commands.forEach((cmd, cmdIndex) => {
                const haystack = (cmd.title + ' ' + cmd.description + ' ' + cmd.syntax).toLowerCase();
        ...
        results.forEach(r => {
            ...
            li.className = 'command-btn' + (isActive ? ' active' : '');
            li.innerHTML = `${r.cmd.title} <span class="command-btn-cat">${r.catName}</span>`;
            li.addEventListener('click', () => {
